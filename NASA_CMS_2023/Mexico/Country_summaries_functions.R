...
Apply_E_FMASK <- function(iso3,aoi,maps,maps_AOImasked,Out_folder){
    ...
    if (iso3=='LBR'){
        ...
        if (!file.exists(RES_name)){
            FMASK_class <- raster(file.path(EMask_folder,EMask_file))
            FMASK_class[!is.finite(FMASK_class)] <- 0
            FMASK_class[FMASK_class <= non_forest_value] <- 0
            if (!identical(crs(FMASK_class), crs("+proj=longlat +datum=WGS84 +no_defs"))){FMASK_class <- projectRaster(FMASK_class, crs=crs("+proj=longlat +datum=WGS84 +no_defs"))}
            writeRaster(FMASK_class, filename=RES_name,datatype="Int16", format="GTiff",options="COMPRESS=LZW",overwrite=TRUE)
        }
    }
    ...
    if (E_FMASK_type=='vector'){
        for (i in 1:length(maps_AOImasked)) {
            ...
            if (!file.exists(Map_with_Fexternal_mask)){
                ...
                input <- raster(Map_with_Fexternal_mask_temp)
                input[input > 49999] <- 0 
                writeRaster(input, filename=Map_with_Fexternal_mask,datatype="FLT4S", format="GTiff", options="COMPRESS=LZW", overwrite=TRUE)
            }
    ...
    if (E_FMASK_type=='raster'){
        for (i in 1:length(maps_AOImasked)) { 
            ...
            dim1 <- dims[[1]]
            dim2 <- dims[[2]]
            extents <- extent(eval(parse(text=maps_AOImasked[i])))
            ex1 <- extents@xmin
            ex2 <- extents@ymin
            ex3 <- extents@xmax
            ex4 <- extents@ymax
            ...
            AOI_masked_country_file <- raster(file.path(Out_folder,paste0(iso3,'_Product_AOI'),paste0(maps[i],"_AOImasked.tif")))
            Map_with_Fexternal_mask <- file.path(Out_folder,paste0(iso3,'_Product_FMASK'),paste0(maps[i],"_EXTERNAL_FMASK_",iso3,".tif"))
            if (!file.exists(Map_with_Fexternal_mask)){
                ...
                FMASK_ready <- raster(sub('_trans','_warps',FMASK_out_name))
                map_Fmasked <- mask(AOI_masked_country_file, FMASK_ready, filename="", inverse=FALSE, maskvalue=0, updatevalue=NA, updateNA=TRUE)
                writeRaster(map_Fmasked, filename=Map_with_Fexternal_mask,datatype="FLT4S", format="GTiff", options="COMPRESS=LZW", overwrite=TRUE)
            }
            maps_FMASKS[[length(maps_FMASKS) + 1]] <- Map_with_Fexternal_mask 
        }
    }
...
agbd_table <- function(year=2020, agb_thresholded, agb_masked){
    ...
    if (!gIntersects(agb_boundpoly_wgs84, aoi_wgs84)) {stop('aoi does not intersect supplied Biomass extract')}
...
overlay_products <- function(Out_folder,maps,maps_AOImasked,AOI_biomes,coarsest_res,CI_map_AOI){
    ...
    writeRaster(base_vals, filename=base_map_name,datatype="FLT4S",overwrite=TRUE)
    base_map <- raster(base_map_name)
    CI <- CI_map_AOI
    base_map_low <- base_map - CI
    base_map_high <- base_map + CI
    set_res <-  res(base_map)
    set_extent <- extent(base_map)
    ...
    for (i in 1:length(maps_AOImasked)){
        if (i != coarsest_res[1]) { 
            ...
            if (!file.exists(output_warps)) {
                ...
                vals <- raster(output_vrt)
                NAvalue(vals) <- 0 
                writeRaster(vals, filename=output_vrt_ND,datatype="FLT4S",overwrite=TRUE)
            ...
            difference <- raster(output_warps)-base_map
            plot(difference, main=paste0(maps[i]," AGBD ", eval(parse(text=paste0(maps[i],"_year"))), ' - ', maps[coarsest_res[1]]," AGBD ", eval(parse(text=paste0(maps[coarsest_res[1]],"_year"))), ' (Mg/ha)'), zlim=c((mean(getValues(difference),na.rm=TRUE)-2*sd(getValues(difference),na.rm=TRUE)),(mean(getValues(difference),na.rm=TRUE)+2*sd(getValues(difference),na.rm=TRUE))),col = pal(50))#;plot(AOI_biomes, add=T)
            ...
            writeRaster(difference, filename=difference_filename,datatype="FLT4S",overwrite=TRUE)
            agb_for_CI_comparison <- raster(output_warps)
    ...
    for (i in 1:length(maps_AOImasked)) { 
        ...
        for (each_file in HANSENlist) {
                ...
                if (!file.exists(RES_name)){
                    ...
                    FMASK_class <- raster(sub('.vrt','.tif',WARP_name))
                    FMASK_class <- reclassify(FMASK_class, rclmat, include.lowest=TRUE)
                    RES_file <- writeRaster(FMASK_class, filename=RES_name,datatype="LOG1S", format="GTiff",options="COMPRESS=LZW",overwrite=TRUE)
                }
        ...
        if (!file.exists(sub('.tif','_warps.tif',HANSEN_country_file)) || !file.exists(Map_with_Hansen_mask)){
            ...
            mosaic_tif <- raster(HANSEN_country_file)          
            ...
            mosaic_tif_cropped_to_AGBmap <- crop(raster(sub('.tif','_trans.tif',HANSEN_country_file)),extent(AOI_masked_country_file),snap='near')
            ...
            Hansen_ready <- raster(sub('.tif','_warps.tif',HANSEN_country_file))
            map_Fmasked <- mask(AOI_masked_country_file, Hansen_ready, filename="", inverse=FALSE, maskvalue=0, updatevalue=NA, updateNA=TRUE)
            writeRaster(map_Fmasked, filename=Map_with_Hansen_mask,datatype="FLT4S", format="GTiff", options="COMPRESS=LZW", overwrite=TRUE)
...
readAGBmap <- function(AOI_file,aoi,continent,map,In_folder,In_year,cci_version,Out_folder,verbose=FALSE){
    ...
    if (each_map == 'ISB'){
        ...
        tile_stack <- mask(raster(file.path(Out_folder,paste0(iso3,'_Product_ISB'),"ISB_AOImasked.tif")), aoi)
        return(tile_stack)
    } else {
        if (each_map == 'CCI'){
            ...
            if (each_map == 'GEDI'){
                file_name <- paste0('GEDI_v21_MU.tif') ###########
                GEDI_NAvalue<- -1
                aoi <- spTransform(aoi, crs("+proj=cea +lat_ts=30 +lon_0=0 +x_0=0 +y_0=0 +datum=WGS84 +units=m"))
            }
            if (each_map == 'GEDI_SE'){
            ...
            skip_to_next <- FALSE
            tryCatch(crop(stack(c(file.path(In_folder,file_name))), aoi), error = function(e) { skip_to_next <<- TRUE})
            if(skip_to_next) { 
                tile_stack <- raster(nrows=5, ncols=5, xmn=0, xmx=1) 
            } else {tile_stack <- crop(stack(c(file.path(In_folder,file_name))), aoi)}
            if (!identical(crs(tile_stack), crs("+proj=longlat +datum=WGS84 +no_defs"))){tile_stack <- projectRaster(tile_stack, crs=crs("+proj=longlat +datum=WGS84 +no_defs"))}
            names(tile_stack) <- Band_name
            NAvalue(tile_stack) <- eval(parse(text=paste0(each_map,'_NAvalue')))
            return(tile_stack)
        }
    }
}
...
fixJPLfile <- function(aoi,verbose=FALSE){
    ...
    for (i in 1:length(All_links)){    
        ...
        if(skip_to_next) { next }  
            ...
            writeRaster(clip,file.path(Out_folder,paste0(iso3,'_Product_AOI'),paste0("Temp_",i,".tif")) ,datatype="FLT4S", format="GTiff", options=c("BIGTIFF=IF_NEEDED","COMPRESSION=LZW","TILED=YES"), overwrite=TRUE)
        }
    }
    ...
    JPL_file <-raster(file.path(Out_folder,paste0(iso3,'_Product_AOI'),paste0("JPL_AOImasked.tif")))
...
readCCI_v2 <- function(data_folder, year=CCI_year, version=CCI_version, aoi, verbose=FALSE){
  ...
  for (n in 1:length(tiles)) {
    ...
    tile_stack <- crop(stack(filenames2), aoi, datatype="FLT4S", format='GTiff', options="COMPRESS=LZW")
